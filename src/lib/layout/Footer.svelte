	...
	const footerNavs = [
...

		{
			...
			items: [
				...
				{
					href: 'https://tahmidchoudhury.vercel.app/',
...
<footer>
	<div class="mx-auto w-full max-w-screen-xl xl:pb-2">
		<div class="gap-4 p-4 px-8 py-16 sm:pb-16 md:flex md:justify-between">
			...
			<div class="grid grid-cols-1 gap-8 sm:grid-cols-3 sm:gap-10">
				{#each footerNavs as nav}
					<div>
						<h2
							class="mb-6 text-sm font-medium uppercase tracking-tighter text-gray-900 dark:text-[var(--black-0)]"
						>
							{nav.label}
						</h2>
						<ul class="grid gap-2">
							{#each nav.items as item}
								<li>
									<a
										href={item.href}
										class="cursor-pointer text-sm font-[450] text-gray-400 duration-200 hover:text-gray-700"
									>
										{item.name}
									</a>
								</li>
		...
		>
			...
			<span class="text-sm text-gray-500 dark:[var(--black-0)] sm:text-center">
				Copyright ©
				{' '}
				{new Date().getFullYear()}
				{' '}
				<a href="/" class="cursor-pointer">Quantum NLP Workshop</a>
				. All Rights Reserved.
			</span>
		</div>
	</div>
